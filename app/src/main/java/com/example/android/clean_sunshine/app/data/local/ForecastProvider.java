...
public class ForecastProvider extends ContentProvider {
        ...
        switch (match) {
            case WEATHER:
                rowsDeleted = db.delete(
                        ForecastContract.WeatherEntry.TABLE_NAME, selection, selectionArgs);
                break;
            case LOCATION:
                rowsDeleted = db.delete(
                        ForecastContract.LocationEntry.TABLE_NAME, selection, selectionArgs);
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        }
    ...
    private void normalizeDate(ContentValues values) {
        ...
        if (values.containsKey(ForecastContract.WeatherEntry.COLUMN_DATE)) {
            ...
            values.put(
                ForecastContract.WeatherEntry.COLUMN_DATE, ForecastContract.normalizeDate(dateValue));
        }
    }
    ...
    public int update(
        ...
        switch (match) {
            case WEATHER:
                normalizeDate(values);
                rowsUpdated = db.update(ForecastContract.WeatherEntry.TABLE_NAME, values, selection,
                        selectionArgs);
                break;
            case LOCATION:
                rowsUpdated = db.update(ForecastContract.LocationEntry.TABLE_NAME, values, selection,
                        selectionArgs);
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
