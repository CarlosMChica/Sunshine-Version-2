...
public class ForecastProvider extends ContentProvider {
  ...
  private static final String sLocationSettingSelection = ForecastContract.LocationEntry.TABLE_NAME +
      "." + ForecastContract.LocationEntry.COLUMN_LOCATION_SETTING + " = ? ";

  //location.location_setting = ? AND date >= ?
  private static final String sLocationSettingWithStartDateSelection = ForecastContract.LocationEntry.TABLE_NAME +
      "." + ForecastContract.LocationEntry.COLUMN_LOCATION_SETTING + " = ? AND " +
      ForecastContract.WeatherEntry.COLUMN_DATE + " >= ? ";

  //location.location_setting = ? AND date = ?
  private static final String sLocationSettingAndDaySelection = ForecastContract.LocationEntry.TABLE_NAME +
      "." + ForecastContract.LocationEntry.COLUMN_LOCATION_SETTING + " = ? AND " +
      ForecastContract.WeatherEntry.COLUMN_DATE + " = ? ";

  private Cursor getWeatherByLocationSetting(Uri uri, String[] projection, String sortOrder) {
    String locationSetting = ForecastContract.WeatherEntry.getLocationSettingFromUri(uri);
    long startDate = ForecastContract.WeatherEntry.getStartDateFromUri(uri);
  ...
  @Override public String getType(Uri uri) {
    ...
    switch (match) {
      ...
      case WEATHER_WITH_LOCATION_AND_DATE:
        return ForecastContract.WeatherEntry.CONTENT_ITEM_TYPE;
      case WEATHER_WITH_LOCATION:
        return ForecastContract.WeatherEntry.CONTENT_TYPE;
      case WEATHER:
        return ForecastContract.WeatherEntry.CONTENT_TYPE;
      case LOCATION:
    ...
    switch (sUriMatcher.match(uri)) {
      ...
      case WEATHER: {
        retCursor = mOpenHelper.getReadableDatabase()
            .query(ForecastContract.WeatherEntry.TABLE_NAME, projection, selection, selectionArgs, null, null,
                sortOrder);
        break;
      }
      // "location"
      case LOCATION: {
        retCursor = mOpenHelper.getReadableDatabase()
            .query(ForecastContract.LocationEntry.TABLE_NAME, projection, selection, selectionArgs, null, null,
                sortOrder);
        break;
      }
  ...
  @Override public Uri insert(Uri uri, ContentValues values) {
    ...
    switch (match) {
      case WEATHER: {
        ...
        long _id = db.insert(ForecastContract.WeatherEntry.TABLE_NAME, null, values);
        if (_id > 0) {
  ...
  @Override public int delete(Uri uri, String selection, String[] selectionArgs) {
    ...
    switch (match) {
      case WEATHER:
        rowsDeleted = db.delete(ForecastContract.WeatherEntry.TABLE_NAME, selection, selectionArgs);
        break;
      case LOCATION:
        rowsDeleted = db.delete(ForecastContract.LocationEntry.TABLE_NAME, selection, selectionArgs);
        break;
      default:
        throw new UnsupportedOperationException("Unknown uri: " + uri);
    }
  ...
  private void normalizeDate(ContentValues values) {
    ...
    if (values.containsKey(ForecastContract.WeatherEntry.COLUMN_DATE)) {
      long dateValue = values.getAsLong(ForecastContract.WeatherEntry.COLUMN_DATE);
      values.put(ForecastContract.WeatherEntry.COLUMN_DATE, ForecastContract.normalizeDate(dateValue));
    }
  }
  ...
  public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
    ...
    switch (match) {
      case WEATHER:
        normalizeDate(values);
        rowsUpdated = db.update(ForecastContract.WeatherEntry.TABLE_NAME, values, selection, selectionArgs);
        break;
      case LOCATION:
        rowsUpdated = db.update(ForecastContract.LocationEntry.TABLE_NAME, values, selection, selectionArgs);
        break;
      default:
        throw new UnsupportedOperationException("Unknown uri: " + uri);
    }
